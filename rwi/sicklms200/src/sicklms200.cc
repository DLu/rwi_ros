...
/*
 ...
 Date: 7 Nov 2000
 CVS: $Id: sicklms200.cc 8062 2009-07-21 03:47:16Z gbiggs $
*/
...
// Macros from PlayerCommon
#define MAKEUINT16(lo, hi) ((((uint16_t) (hi)) << 8 ) | ((uint16_t) (lo)))
#define DTOR(d) angles::from_degrees(d)
...
////////////////////////////////////////////////////////////////////////////////
// Error macros
#define RETURN_ERROR(erc, m) {ROS_ERROR(m); return erc;}
...
SickLMS200::SickLMS200() {
    // Serial port
    std::string port;
    n.param<std::string>("port", port, DEFAULT_LASER_PORT);
    ROS_INFO("port: %s", port.c_str());
    this->device_name = new char[port.length()];
    strcpy(this->device_name, port.c_str());
    n.param("plsmode", this->PLSMode, false);

    // Serial rate
	if(n.hasParam("connect_rate")){
    	XmlRpc::XmlRpcValue xml_connect_rates = NULL;
    	n.param("connect_rate", xml_connect_rates, xml_connect_rates);
    	ROS_INFO("%s", xml_connect_rates.toXml().c_str());
    	if (xml_connect_rates.getType() == XmlRpc::XmlRpcValue::TypeArray) {
        	this->num_connect_rates = xml_connect_rates.size();
        	for (int i = 0; i < this->num_connect_rates; ++i) {
            	std::string rate = xml_connect_rates[i];
            	this->connect_rates[i] = atoi(rate.c_str());
        	}
		
    } else {
        ROS_WARN("connect rate is not an array");
    }
	}else{
    ...
    this->current_rate = 0;
    n.param("retry", this->retry_limit, 0);
    this->retry_limit += 1;

#ifdef HAVE_HI_SPEED_SERIAL
    n.param("serial_high_speed_mode", this->serial_high_speed_mode, 0);
#else
    n.param("serial_high_speed_mode", this->serial_high_speed_mode, 1);
#endif

    n.param("serial_high_speed_baudremap", this->serial_high_speed_baudremap, 38400);
    ...
    n.param("delay", this->startup_delay, 0);
    this->scan_width = 180;
    n.param("resolution", this->scan_res, 50);
    if ((this->scan_res != 25) &&
            (this->scan_res != 50) &&
            (this->scan_res != 100)) {
        ROS_ERROR("Invalid angular resolution %d. Defaulting to 50 (0.5 degree)",
                  this->scan_res);
        this->scan_res = 50;
    }
    this->min_angle = -9000;
    ...
    this->intensity = true;
    n.param("range_res", this->range_res, 1);
    n.param("invert", this->range_res, 0);
    n.param("high_avail", this->range_res, 0);

    if (this->CheckScanConfig() != 0)
        ROS_ERROR("invalid scan configuration");


    laser_pub = n.advertise<sensor_msgs::LaserScan>("laser_scan", 1);
    return;
}
...
int SickLMS200::MainSetup() {
    ROS_INFO("Laser initialising (%s)", this->device_name);
    ...
    for (int i=0;i<this->retry_limit;i++) {
        ...
        for (int j=0;j<this->num_connect_rates;j++) {
            ...
            if (this->current_rate != 0) {
                ROS_INFO("connected at %d!", this->current_rate);
                break;
            }
        }
        if (this->current_rate != 0)
    ...
    if (this->current_rate == 0) {
        ROS_ERROR("unable to connect to laser");
        return 1;
    }

    sleep(1);
    if (this->current_rate != this->transfer_rate && (this->transfer_rate == 38400 || this->transfer_rate == 500000) ) {
        ROS_INFO("laser operating at %d; changing to %d", this->current_rate, this->transfer_rate);
    ...
    // Dont know this rate
        ...
        ROS_ERROR("unsupported transfer rate %d", this->transfer_rate);
        return 1;
    }
...
//  char type[64];
    ...
    if (PLSMode) {
        ...
        ROS_INFO("SICK laser type [%s] at [%s:%d]", this->laser_type, this->device_name, this->transfer_rate);
...
void SickLMS200::MainQuit() {
    ...
    if (this->connect_rate != this->current_rate)
        ...
        if (SetLaserSpeed(this->connect_rate))
            ROS_WARN("Cannot throttle back to %d bauds", this->connect_rate);

    CloseTerm();

    ROS_INFO("laser shutdown");
}
...
ProcessMessage()
{
                           ...
                           this->device_addr))
  {
    ...
    if(this->CheckScanConfig() != 0)
    {
      ROS_ERROR("invalid laser configuration requested");
      return(-1);
    }
    if (SetLaserMode() != 0)
      ROS_ERROR("request for config mode failed");
    else
    {
      if (SetLaserRes(this->scan_width, this->scan_res) != 0)
	ROS_ERROR("failed setting resolution [SetLaserRes()]");

      if(SetLaserConfig(this->intensity, this->high_avail) != 0)
        ROS_ERROR("failed setting configuration [SetLaserConfig()]");
    }

    // Issue a new request for data
    if (RequestLaserData(this->scan_min_segment, this->scan_max_segment))
      ROS_ERROR("request for laser data failed");
...
void SickLMS200::Main() {
    ...
    if (RequestLaserData(this->scan_min_segment, this->scan_max_segment) != 0) {
        ROS_ERROR("laser not responding; exiting laser thread");
        return;
    }

    int hz;
    n.param("rflex_rate", hz, 30);
    ...
    while (ros::ok()) {
        ...
        if (ReadLaserData(mm_ranges, 1024) == 0) {
            if (first) {
                ROS_INFO("SickLMS200: receiving data");
                first = false;
            }
            ...
            // Prepare packet
            data.angle_min = DTOR((this->scan_min_segment *
                                   this->scan_res) / 1e2 - this->scan_width / 2.0);
            data.angle_max = DTOR((this->scan_max_segment *
                                   this->scan_res) / 1e2  -
                                  this->scan_width / 2.0);
            if (this->range_res == 1)
                data.range_max = 8.0;
                ...
                data.range_max = 150.0;
            else {
                ROS_WARN("Invalid range_res!");
                data.range_max = 8.0;
            }
            data.angle_increment = DTOR(this->scan_res / 1e2);
            int ranges_count = this->scan_max_segment - this->scan_min_segment + 1;
            data.set_ranges_size(ranges_count);
            data.set_intensities_size(ranges_count);

            for (int i = 0; i < this->scan_max_segment - this->scan_min_segment + 1; i++) {
                /* Not sure about this */
                if (strncmp(this->laser_type, "PLS", 3) == 0)
                    data.intensities[i] = ((mm_ranges[i] >> 13) & 0x000E);
                else
                    data.intensities[i] = ((mm_ranges[i] >> 13) & 0x0007);
            ...
            if (this->invert) {
                for (int i = 0;
                    ...
                    itmp=data.intensities[i];
                    data.intensities[i]=data.intensities[this->scan_max_segment-this->scan_min_segment-i];
                    data.intensities[this->scan_max_segment-this->scan_min_segment-i] = itmp;
                }
            }

            data.header.seq = this->scan_id++;

            // Make data available
            this->laser_pub.publish(data);
        }
...
#endif
    if (this->laser_fd < 0) {
        ROS_ERROR("unable to open serial port [%s]; [%s]",
...
int SickLMS200::CloseTerm() {
    ...
    if (ioctl(this->laser_fd, TIOCSSERIAL, &this->old_serial) < 0) {
      ...
      ROS_WARN("ioctl() failed while trying to reset serial to old state");
    }
    #endif
    */
...
int SickLMS200::ChangeTermSpeed(int speed) {
    ...
    if (this->serial_high_speed_mode == 0) {
        ...
        if (ioctl(this->laser_fd, TIOCGSERIAL, &serial) < 0) {
            ...
            ROS_WARN("ioctl() failed while trying to get serial port info");
        } else {
            serial.flags &= ~ASYNC_SPD_CUST;
            serial.custom_divisor = 0;
            if (ioctl(this->laser_fd, TIOCSSERIAL, &serial) < 0) {
                //RETURN_ERROR(1, "error on TIOCSSERIAL in beginning");
                ROS_WARN("ioctl() failed while trying to set serial port info");
            }
        }
    }
#endif
    ...
    switch (speed) {
    case 9600:
        //ROS_INFO("terminal speed to 9600");
    ...
    case 38400:
        //ROS_INFO("terminal speed to 38400");
    ...
    case 500000:
        //ROS_INFO("terminal speed to 500000");
        if (this->serial_high_speed_mode == 0) {
#ifdef HAVE_HI_SPEED_SERIAL
            ...
            //Using the ioctl() API to set non-standard baud rate after setting the termios parameters.
#define  MOXA_SET_SPECIAL_BAUD_RATE  0x44D
    ...
    default:
        ROS_ERROR("unknown speed %d", speed);
    }
    return 0;
}
...
int SickLMS200::SetLaserMode() {
    ...
    for (tries = 0; tries < DEFAULT_LASER_RETRIES; tries++) {
        ...
        */
        ...
        ROS_INFO("sending configuration mode request to laser [SetLaserMode()]");
        ...
        //
        ROS_INFO("waiting for acknowledge");
        len = ReadFromLaser(packet, sizeof(packet), true, 2500, 250);
        if (len < 0)
            return 1;
        else if (len < 1) {
            ROS_WARN("SetLaserMode(): timeout in ReadFromLaser");
            continue;
        } else if (packet[0] == NACK) {
            ROS_ERROR("SetLaserMode(): request denied by laser");
            return 1;
        } else if (packet[0] != ACK) {
            ROS_ERROR("SetLaserMode(): unexpected packet type");
            return 1;
        }
        break;
    }
...
int SickLMS200::SetLaserSpeed(int speed) {
    ...
    //ROS_INFO("sending baud rate request to laser");
    if (WriteToLaser(packet, len) < 0)
        return 1;
    ...
    if (len < 0)
        ...
        ROS_ERROR("SetLaserSpeed(): no reply from laser");
        return 1;
    } else if (packet[0] == NACK) {
        ROS_ERROR("SetLaserSpeed(): request denied by laser");
        return 1;
    } else if (packet[0] != ACK) {
        ROS_ERROR("SetLaserSpeed(): unexpected packet type");
        return 1;
    }
    return 0;
}
...
int SickLMS200::GetLaserType(char *buffer, size_t bufflen) {
    ...
    for (tries = 0; tries < DEFAULT_LASER_RETRIES; tries++) {
        ...
        //ROS_INFO("sending get type request to laser");
        if (WriteToLaser(packet, len) < 0)
            return 1;
        ...
        if (len < 0)
            ...
            ROS_WARN("GetLaserType(): timeout in ReadFromLaser()");
            continue;
        } else if (packet[0] == NACK) {
            ROS_ERROR("GetLaserType(): request denied by laser");
            return 1;
        } else if (packet[0] != 0xBA) {
            ROS_ERROR("GetLaserType(): unexpected packet type");
            return 1;
        }
...
int SickLMS200::GetLaserConfig(uint8_t* packet, ssize_t packet_size) {
    ...
    for (tries = 0; tries < DEFAULT_LASER_RETRIES; tries++) {
        ...
        //ROS_INFO("sending get configuration request to laser");
        if (WriteToLaser(packet, len) < 0)
            return 1;

        // Wait for laser to return data
        //ROS_INFO("waiting for reply");
        len = ReadFromLaser(packet, packet_size, false, 25000, 5000);
        if (len < 0)
            return 1;
        else if (len < 1) {
            ROS_WARN("GetLaserConfig(): timeout in ReadFromLaser() [1]");
            continue;
        } else if (packet[0] == NACK) {
            ROS_ERROR("GetLaserConfig(): request denied by laser [1]");
            return 1;
        } else if (packet[0] != 0xF4) {
            ROS_ERROR("GetLaserConfig(): unexpected packet type [1]");
            return 1;
        }
        break;
    }
...
int SickLMS200::SetLaserConfig(bool intensity, bool high_avail) {
    ...
    if (CheckLaserConfig(packet,sizeof(packet),
        ...
        ROS_INFO("No need to flash redundant configuration to device.");
        return 0;
    }

    //ROS_INFO("get configuration request ok");
    //PLAYER_TRACE1("laser units [%d]", (int) packet[7]);

    ROS_INFO("Flashing new configuration to device.");
    ...
    for (tries = 0; tries < DEFAULT_LASER_RETRIES; tries++) {
        ...
        //ROS_INFO("sending set configuration request to laser");
        if (WriteToLaser(packet, len) < 0)
            return 1;
        ...
        if (len < 0)
            ...
            ROS_WARN("SetLaserConfig(): timeout in ReadFromLaser() [2]");
            continue;
        } else if (packet[0] == NACK) {
            ROS_ERROR("SetLaserConfig(): request denied by laser [2]");
            return 1;
        } else if (packet[0] != 0xF7) {
            ROS_ERROR("SetLaserConfig(): unexpected packet type [2]");
            return 1;
        }

        //ROS_INFO("set configuration request ok");
        break;
    }
...
int SickLMS200::GetLaserStatus(uint8_t* packet, ssize_t packet_size) {
    ...
    for (tries = 0; tries < DEFAULT_LASER_RETRIES; tries++) {
        ...
        //ROS_INFO("sending get status request to laser");
        if (WriteToLaser(packet, len) < 0)
            return 1;
        ...
        if (len < 0)
            ...
            ROS_WARN("GetLaserStatus(): timeout in ReadFromLaser() [1]");
            continue;
        } else if (packet[0] == NACK) {
            ROS_ERROR("GetLaserStatus(): request denied by laser [1]");
            return 1;
        } else if (packet[0] != 0xB1) {
            ROS_ERROR("GetLaserStatus(): unexpected packet type [1]");
            return 1;
        }
        break;
    }
...
int SickLMS200::SetLaserRes(int width, int res) {
    ...
    if (CheckLaserRes(packet,sizeof(packet),
                      width, res) == 0) {
        ROS_INFO("No need to send redundant resolution to device.");
        return 0;
    }

    ROS_INFO("Sending new resolution to device.");
    ...
    for (tries = 0; tries < DEFAULT_LASER_RETRIES; tries++) {
        ...
        //ROS_INFO("sending set variant request to laser");
        if (WriteToLaser(packet, len) < 0)
            return 1;
        ...
        if (len < 0)
            ...
            ROS_ERROR("request denied by laser");
            return 1;
        } else if (packet[0] != 0xBB) {
            ROS_ERROR("unexpected packet type");
            return 1;
        }

        // See if the request was accepted
        if (packet[1] == 0) {
            ROS_ERROR("variant request ignored");
            return 1;
        }
...
int SickLMS200::RequestLaserStopStream() {
    ...
    for (tries = 0; tries < DEFAULT_LASER_RETRIES; tries++) {
        ...
        packet[1] = 0x25;
        len = 2;
        ROS_INFO("sending LMS stop continuous mode [RequestLaserStopStream()]");
        if (WriteToLaser(packet, len) < 0)
            return 1;
        ...
        if (len < 0)
            ...
            ROS_WARN("RequestLaserStopStream(): timeout in ReadFromLaser -- may be using wrong baud rate.");
            continue;
        } else if (packet[0] == NACK) {
            ROS_ERROR("RequestLaserStopStream(): request denied by laser");
            return 1;
        } else if (packet[0] != ACK) {
            ROS_ERROR("RequestLaserStopStream(): unexpected packet type");
            return 1;
        }
        break;
    }
...
int SickLMS200::RequestLaserData(int min_segment, int max_segment) {
    ...
    for (tries = 0; tries < DEFAULT_LASER_RETRIES; tries++) {
        ...
        //ROS_INFO("sending scan data request to laser");
...
ssize_t SickLMS200::ReadFromLaser(uint8_t *data, ssize_t maxlen, bool ack, int timeout, int timeout_header) {
    ...
    //ROS_INFO("%Ld %Ld", start_time, stop_time);
    ...
    while (true) {
        if (timeout >= 0) {
            ...
            if (retval < 0) {
                if (errno == EINTR)
                    continue;
                ROS_ERROR("error on select (1)");
                return 0;
            }
            if (!retval) {
                ROS_INFO("timeout on select (1)");
                return 0;
            }
        }
    ...
    while (bytes < len) {
        if (timeout >= 0) {
            ...
            if (retval < 0) {
                if (errno == EINTR)
                    continue;
                ROS_ERROR("error on select (3)");
                return 0;
            }
            if (!retval) {
                ROS_INFO("timeout on select (3)");
                return 0;
            }
        }
        ...
        if (retval < 0) {
            if (errno == EINTR)
                continue;
            ROS_ERROR("error on read (3)");
            return 0;
        }
        if (!retval) {
            ROS_INFO("eof on read (3)");
            return 0;
        }
        bytes += retval;
    }
    ...
    while (bytes < 3) {
        if (timeout >= 0) {
            ...
            if (retval < 0) {
                if (errno == EINTR)
                    continue;
                ROS_ERROR("error on select (4)");
                return 0;
            }
            if (!retval) {
                ROS_INFO("timeout on select (4)");
                return 0;
            }
        }
        ...
        if (retval < 0) {
            if (errno == EINTR)
                continue;
            ROS_ERROR("error on read (4)");
            return 0;
        }
        if (!retval) {
            ROS_INFO("eof on read (4)");
            return 0;
        }
        bytes += retval;
    }
...
int main(int argc, char** argv) {
    ...
    SickLMS200* node = new SickLMS200();
    if (node->MainSetup()!=0) {
